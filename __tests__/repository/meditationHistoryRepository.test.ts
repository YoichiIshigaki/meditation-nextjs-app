...
describe('getMeditationHistoryMaxConsecutiveDays', () => {
  ...
  describe('空の履歴の場合', () => {
    it('履歴が空の場合は0を返す', async () => {
      ...
      const result = await getMeditationHistoryMaxConsecutiveDays(mockUserId, 'all_time');
  ...
  describe('連続した日付の場合', () => {
    it('3日連続の場合は3を返す', async () => {
      ...
      const mockHistories = createConsecutiveDaysMockHistories(mockUserId, today, 3);
    ...
    it('5日連続の場合は5を返す', async () => {
      const today = new Date();
      const mockHistories = createConsecutiveDaysMockHistories(mockUserId, today, 5);
  ...
  describe('連続が途切れている場合', () => {
    it('3日連続と2日連続がある場合、最大値の3を返す', async () => {
      ...
      // 新しい期間（3日連続）
      const newPeriod = createMockHistoriesFromDates(mockUserId, [day2, day1, today]);
  ...
  describe('複雑なシナリオ', () => {
    it('複数の連続期間がある場合、最大値を返す', async () => {
      ...
      const newPeriod = createConsecutiveDaysMockHistories(
        ...
        today,
        3,
        { idPrefix: 'new', baseDuration: 15, baseScore: 90, baseMindfulnessScore: 85 }
      );
