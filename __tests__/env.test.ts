...
describe('環境変数ファイルの検証', () => {
      ...
      test(`${key} が少なくとも1つの.envファイルに存在すること`, () => {
        ...
        const foundInFiles = envFiles
          .filter((file) => {
            ...
            return aliases.some((alias) => isKeyValid(alias, file.content[alias]));
          })
        ...
        REQUIRED_ENV_KEYS.forEach((key) => {
          test(`${key} が存在し、空でないこと（エイリアスも含む）`, () => {
            ...
            if (foundAlias) {
              ...
              expect(isKeyValid(foundAlias, file.content[foundAlias])).toBe(true);
            } else {
  ...
  describe('環境変数ファイルの存在確認', () => {
    ...
    // 主要な環境ファイルの存在を確認
    const essentialFiles = ['.env', '.env.local', '.env.development', '.env.production'];
    essentialFiles.forEach((pattern) => {
      ...
      test(`${pattern} が存在するか、または他の.envファイルでカバーされていること`, () => {
        ...
        if (!exists) {
          ...
          const allKeys = new Set(envFiles.flatMap((file) => Object.keys(file.content)));
