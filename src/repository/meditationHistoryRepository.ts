...
const getThisMonthMeditationHistories = async (
  userId: string,
): Promise<MeditationHistory[]> => {
  const now = new Date();
  return await meditationHistory.list({
...
const getAllTimeMeditationHistories = async (
  userId: string,
): Promise<MeditationHistory[]> => {
  return await meditationHistory.list({
    userId,
 ...
 */
const getHistoryByPeriod = async (
  userId: string,
  type: PeriodType,
): Promise<MeditationHistory[]> => {
 ...
 */
const sortHistoriesByDateAsc = (
  histories: MeditationHistory[],
): MeditationHistory[] => {
